! function(t, e) {
	...
	var n, o, l = (n = [], o = {}, new(function() {
		...
		h = function() {
			...
			return a(i, null, [{
				...
				value: function(tree_data) {
					var tree = d3.layout.tree().separation((function(t, e) {
							return t.parent, e.parent, 1
						})),
					i = tree.nodes(tree_data);
		...
		c = function() {
			function t(i) {
				...
				var a = -35;
				this.textNodes.train.accuracy.text("0")
				  .attr("text-anchor", "middle")
				  .attr("y", a)
				  .attr("font-size", 26)
				  .attr("fill", "#000000"), 
				...
				this.trainingNotTargetDivider = this.trainingGroup.append("text")
				  .text("/")
				  .attr("text-anchor", "middle")
				  .attr("y", a).attr("font-size", 22), 
				this.textNodes.train.isTarget.correct.text("0")
				  .attr("text-anchor", "end")
				  .attr("fill", h.FILL_FN(!0))
				  .attr("y", a).attr("font-size", 22),
				this.textNodes.train.isTarget.total.text("0")
				  .attr("text-anchor", "start")
				  .attr("fill", h.FILL_FN(!0))
				  .attr("y", a).attr("font-size", 22),
				this.textNodes.train.isNotTarget.correct.text("0")
				  .attr("text-anchor", "end")
				  .attr("fill", h.FILL_FN(!1))
				  .attr("y", a)
				  .attr("font-size", 22), 
				this.textNodes.train.isNotTarget.total.text("0")
				  .attr("text-anchor", "start")
				  .attr("fill", h.FILL_FN(!1)).attr("y", a)
				  .attr("font-size", 22), 
				...
				this.textNodes.test.accuracy.text("0")
				  .attr("text-anchor", "middle")
				  .attr("y", a)
				  .attr("font-size", 26)
				  .attr("fill", "#333333"), 
				...
				this.testNotTargetDivider = this.testGroup.append("text")
				  ...
				  .attr("y", a)
				  .attr("font-size", 22), 
				this.textNodes.test.isTarget.correct.text("0")
				  .attr("text-anchor", "end")
				  .attr("fill", h.FILL_FN(!0))
				  .attr("y", a)
				  .attr("font-size", 22), 
				this.textNodes.test.isTarget.total.text("0")
				  .attr("text-anchor", "start")
				  .attr("fill", h.FILL_FN(!0))
				  .attr("y", a)
				  .attr("font-size", 22), 
				this.textNodes.test.isNotTarget.correct.text("0")
				  .attr("text-anchor", "end")
				  .attr("fill", h.FILL_FN(!1))
				  .attr("y", a)
				  .attr("font-size", 22), 
				this.textNodes.test.isNotTarget.total.text("0")
				  .attr("text-anchor", "start")
				  .attr("fill", h.FILL_FN(!1))
				  .attr("y", a)
				  .attr("font-size", 22), 
				...
				this.rr = !0, i.renderer.views.push(this)
			}
			return a(t, [{
				key: "handleResize",
				value: function(t) {
					...
					this.width < 500 ? (this.largeTextSize = 20, this.smallTextSize = 16) 
						: (this.largeTextSize = 24, this.smallTextSize = 20),
					this.largeTextNodes.forEach(function(t) {
						...
						this.labelVerticalOffset = -this.largeTextSize - 10, 
						this.testLabel.attr("y", this.labelVerticalOffset-20), 
						this.trainingLabel.attr("y", this.labelVerticalOffset-20);
				...
				value: function() {
					...
					this.textNodes.train.accuracy.text(t + "%"), isNaN(t) ? this.textNodes.train.accuracy.attr("opacity", 0) : this.textNodes.train.accuracy.attr("opacity", 1), 
					...
					this.textNodes.test.accuracy.text(e + "%"), isNaN(e) ? this.textNodes.test.accuracy.attr("opacity", 0) : this.textNodes.test.accuracy.attr("opacity", 1), 
		...
		d = function() {
			function t(i) {
				...
				this.tree.nodes.forEach(function(t) {
					var e, i, labelLayer = this.labelsGroup.append("g"),
						overlayLayer = labelLayer.append("g"),
						textLayer = overlayLayer.append("g"),
						hoverLayer = this.hoverGroup.append("g");
					t.key ? (e = t.value, i = t.key) : (e = "", i = "");
					var leftText = textLayer.append("text");
					leftText
					  .attr("text-anchor", "end")
					  .attr("x", 3 * this.ballSpacing)
					  .attr("font-size", 15)
					  .attr("y", 56)
					  .text(i + " <= " + e);
					...
					rightText
					  ...
					  .attr("y", 36)
					  .text(i + " > " + e);
					var h, c, d = labelLayer.append("g"),
						u = labelLayer.append("g"),
						p = d.append("path").attr("fill", "none").attr("stroke-width", 1),
						f = u.append("path").attr("fill", "none").attr("stroke-width", 1),
						g = labelLayer.append("rect").attr("opacity", 0),
						y = labelLayer.append("rect").attr("opacity", 0);
					t.children && (
						h = hoverLayer
							  .append("rect")
						      .classed("leftHover", !0)
						      .attr("x", -60).attr("width", 60)
						      .attr("y", .6 * this.labelOffset)
						      .attr("height", 60).attr("opacity", 0)
			...
			return a(t, [{
				...
				value: function(t, e, i) {
					...
					e.forEach(function(t) {
						...
						t.classification = i, 
						t.attributes.target > .5 ? t.correctlyClassified = "isTarget" === t.classification : t.correctlyClassified = "isNotTarget" === t.classification, t.correctlyClassified || t.circle.attr("fill", "none")
				...
				value: function() {
					...
					this.svg.attr("width", this.bounds.width - this.paddingRight)
					  .attr("height", this.bounds.height);
					var treeExtent = {
						x: [90, this.bounds.width - this.paddingRight - 40],
						y: [.05 * this.bounds.height + this.ballSpacing, .5 * this.bounds.height]
					};
					this.treeScales = {
					...
					this.splitLabels.forEach(function(t) {
						...
						if(t.node.children) {
							...
							x += "L " + p + " " + g + " ", x += "L " + p + " " + (this.treeScales.y(t.node.children[0].y) + this.labelOffset - this.ballSpacing) + " ", t.leftLine.attr("d", x);
							var b = "";
					...
					this.baseline = {}, 
					this.baseline.training = .93 * this.bounds.height+11, 
					this.baseline.test = this.baseline.training - 15 * this.ballSpacing;
					...
					// Scroll Stuff
					var trainingStart = this.trainingSection.offsetTop - .1 * t,
						trainingEnd = 3.9 * t,
						testStart = this.testSection.offsetTop,
						testEnd = testStart + 1.5 * t,
						ballDurationInPixels = Math.round(t / 4),
						trainingInterval = Math.floor((trainingEnd - trainingStart - ballDurationInPixels) / this.trainingSamples.length),
						testInterval = Math.floor((testEnd - testStart - ballDurationInPixels) / this.testSamples.length);
					this.trainingSamples.forEach((function(t, e) {
						var i = trainingStart + e * trainingInterval,
							a = trainingStart + ballDurationInPixels;
						t.scrollExtent = [i, a], t.scrollDuration = ballDurationInPixels
					})), 
					this.testSamples.forEach((function(t, e) {
						var i = testStart + e * testInterval,
							a = testStart + ballDurationInPixels;
						t.scrollExtent = [i, a], t.scrollDuration = ballDurationInPixels
					})), 
	...
	return function() {
		function t(i) {
			...
			this.data = null, this.isInstall = !1, 
		...
		return a(t, [{
			...
			key: "play",
			value: function() {
				if(!this.isInstall) return console.error("Not Installed!"), 
